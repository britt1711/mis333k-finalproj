...
namespace fa21team9finalproject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ChangePhoneNumber(string id, [Bind("Email,PhoneNumber")] AppUser user)
        {
            ...
            {
                return View("Error", new String[] { "Phone number cannot be left empty." }); 
            }
        ...
        public async Task<IActionResult> HostDeleteAccount()
        {
            ...
            List<Property> properties = _context.Properties.Include(p => p.Reservations).ThenInclude(r => r.Order).Where(r => r.HostEmail == user.Email).ToList();
            foreach(Property property in properties)
            {
                property.Status = pStatus.notActive;
                _context.Update(property);

                List<Reservation> reservations = _context.Reservations.Include(r => r.Order).ThenInclude(r => r.User).Where(r => r.Property.PropertyID == property.PropertyID).ToList();
                foreach(Reservation r in reservations)
                {
                    if (r.Status == rStatus.Active && r.CheckInDate >= DateTime.Now)
                    {
                        r.Status = rStatus.Cancelled;
                        String ConfirmationNum = "Your confirmation number is: " + r.Order.OrderNumber.ToString();
                        ...
                        Utilities.EmailMessaging.SendEmail(r.Order.User.Email, eSubject, eBod + ConfirmationNum);
                    }
